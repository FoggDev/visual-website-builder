...
function PagePreviewContent() {
  ...
  useEffect(() => {
    ...
    const fallbackPage =
      matchingPage || state.pages.find((page) => page.url === pageUrl)

    if (fallbackPage && fallbackPage.id !== state.currentPageId) {
      dispatch({
        type: 'SET_CURRENT_PAGE',
        payload: { pageId: fallbackPage.id }
      })
    }
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center space-y-4">
          <div className="text-yellow-600 bg-yellow-50 border border-yellow-200 rounded-lg p-4 max-w-md">
            ...
            <p className="text-sm">
              No page at {pageUrl} detected. You can configure custom routes in
              the builder.
            </p>
          </div>
